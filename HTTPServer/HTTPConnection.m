...
#if TARGET_OS_IPHONE
  #define POST_CHUNKSIZE  (1024 * 128)//一次1K太少(1024 * 1)，改成(1024 * 1024)
//#define POST_CHUNKSIZE  (1024 * 1)//一次1K太少(1024 * 1)，改成(1024 * 1024)
#else
...
#define TIMEOUT_READ_BODY                    -1  ////-1
#define TIMEOUT_WRITE_HEAD                   30
#define TIMEOUT_WRITE_BODY                   -1  /////-1
...
- (void)replyToHTTPRequest
{
    ...
    NSLog(@"reply method %@",request.method);
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
	...
	if (tag == HTTP_REQUEST_HEADER)
	{
		...
		else
		{
			...
			NSLog(@"method   %@,uri  %@,期望upload=%d,contentLength=%@",method,uri,expectsUpload,contentLength);
			if (expectsUpload)
			{
				...
				if (requestContentLength > 0)
				{
					...
					//[asyncSocket readDataToLength:bytesToRead
					                  //withTimeout:TIMEOUT_READ_BODY
					                          //tag:HTTP_REQUEST_BODY]; //跑入一个判断length的方法
                    [asyncSocket readDataWithTimeout:TIMEOUT_READ_BODY
                                                 tag:HTTP_REQUEST_BODY];//跑入另一length=0也不会return的方法
				}
				else
				{
					// Empty upload
                    NSLog(@"因为requestContentLength= %lld 而replyToHTTPRequest",requestContentLength);
			...
			else
			{
                ...
                NSLog(@"因为method为 get  而replyToHTTPRequest");
				[self replyToHTTPRequest];
			}
		}
	}
	else
	{
		...
		else
		{
			...
			// Now we need to reply to the request
            NSLog(@"数据彻底拿成功，而replyToHTTPRequest");
            //重要，zxzzxz，这里改为RTSP解码数据流就0K啦
//			[self replyToHTTPRequest];
            if ([[request version]  isEqualToString:HTTPVersion1_1] ||[[request version] isEqualToString:HTTPVersion1_0])
            {
                [self replyToHTTPRequest];
                return;
            }
           
            NSLog(@"%@",[[NSString alloc] initWithData:[request messageData] encoding:NSASCIIStringEncoding]);
            NSString *tmp = [[NSString alloc] initWithData:[request messageData] encoding:NSASCIIStringEncoding];
            ...
            [options appendFormat:@"RTSP/1.0 200 OK%@",KENTER];
            [options appendFormat:@"video-Latency: 90%@",KENTER];
            [options appendFormat:@"Server: AirPlay/267.3%@",KENTER];
            [options appendFormat:@"CSeq: 1%@%@",KENTER,KENTER];
...
//            if ([socketudp connectToAddress:[sock connectedAddress] error:nil ]) {
//            
////                [socketudp sendData:[request messageData] toAddress:[sock connectedAddress]  withTimeout:1 tag:1];
//                [socketudp sendData:[options dataUsingEncoding:NSASCIIStringEncoding] toAddress:[sock connectedAddress]  withTimeout:1 tag:1];
...
//            ;
            ...
            if ([asyncSocket connectToHost:[sock connectedHost] onPort:[sock connectedPort] error:nil]) {
