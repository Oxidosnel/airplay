...
#define MAIN_Cache_Path [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0]

-(void)appendsave{
    

    NSLog(@"appendsave!!!!");
    //NSString *file=[MAIN_Cache_Path stringByAppendingPathComponent:@"图片"];
    UIImage *img1=[UIImage imageWithData:LONdATA];
    
    AirViewAppDelegate *app=(AirViewAppDelegate *)[[UIApplication sharedApplication] delegate];
    [app.showAndMonitorView showImage:img1];
    
    
    //[UIImagePNGRepresentation(img1) writeToFile: file   atomically:YES];
    //[UIImageJPEGRepresentation(img1, 1.0) writeToFile: file   atomically:YES];
}
- (void)processDataChunk:(NSData *)postDataChunk
{
    ...
    if (!LONdATA) {
        LONdATA=[[NSMutableData alloc] init];
        
    }
    [LONdATA appendData:postDataChunk];
   
    if (LONdATA.length==requestContentLength) {
        //[self  appendsave];
        [self performSelector:@selector(appendsave) onThread:[NSThread mainThread] withObject:nil waitUntilDone:YES];
    }
...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
    ...
    NSLog(@"%@[%p]: %@ (%qu) %@", THIS_FILE, self, method, requestContentLength, path);
...
//    
//        NSLog(@"%@",request);
//    }else if([path isEqualToString:@"/pair-verify"]){
//        NSLog(@"%@",request);
//    }
    else
	if ([path isEqualToString:@"/reverse"])
